...
class BSTRO_BodyHSC_Network(torch.nn.Module):
    ...
    def __init__(self, args, config, backbone, trans_encoder, mesh_sampler):
        super(BSTRO_BodyHSC_Network, self).__init__()
        self.config = config
        ...
        self.conv_learn_tokens = torch.nn.Conv1d(49,431,1)
    ...
    def forward(self, images, smpl, mesh_sampler, meta_masks=None, is_train=False):
        ...
        if self.config.output_attentions==True:
            return torch.sigmoid(pred_vertices_sub2), torch.sigmoid(pred_vertices_sub), torch.sigmoid(pred_vertices_full), hidden_states, att
        else:
            return torch.sigmoid(pred_vertices_sub2), torch.sigmoid(pred_vertices_sub), torch.sigmoid(pred_vertices_full)